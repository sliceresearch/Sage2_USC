...
function omicronManager( inConfig )
{
	omicronManager = this;
	config = inConfig.experimental.omicron;
	...
	server.listen(dataPort, inConfig.host);
}
					...
					*/
					...
					if( e.flags == FLAG_SINGLE_TOUCH )
					{
						...
						if(address in sagePointers){
							//server.showPointer( address, { label:  "Touch: " + sourceID, color: "rgba(255, 255, 255, 1.0)" } );
						}else{
							//server.createSagePointer(address);
							
							//server.showPointer( address, { label:  "Touch: " + sourceID, color: "rgba(255, 255, 255, 1.0)" } );
							
							//server.pointerPress( address, posX, posY );
						}
					}
