...
appLoader.prototype.loadApplicationFromRemoteServer = function(application, callback) {
	var mime_type = application.type;
	var app = this.mime2app[mime_type];
	var strictSSL = false;
	var filename = decodeURI(application.url.substring(application.url.lastIndexOf("/")+1));
	
	if(app === "image_viewer"){
		this.loadImageFromURL(application.url, mime_type, filename, strictSSL, function(appInstance) {
			callback(appInstance);
		});
	}
	else if(app === "movie_player"){
		if(mime_type === "video/youtube"){
			this.loadYoutubeFromURL(application.url, function(appInstance) {
				callback(appInstance);
			});
		}
		else{
			this.loadVideoFromURL(application.url, mime_type, application.url, filename, function(appInstance) {
				callback(appInstance);
			});
		}
	}
	else if(app === "pdf_viewer"){
		this.loadPdfFromURL(application.url, mime_type, filename, strictSSL, function(appInstance) {
			callback(appInstance);
		});
	}
}
...
appLoader.prototype.loadFileFromWebURL = function(file, callback) {
	...
	if(app === "image_viewer"){
		this.loadImageFromURL(file.url, mime_type, filename, strictSSL, function(appInstance) {
			callback(appInstance);
		});
	}
	else if(app === "movie_player"){
		if(mime_type === "video/youtube"){
			...
			this.loadVideoFromURL(file.url, mime_type, file.url, filename, function(appInstance) {
				callback(appInstance);
			});
		}
	}
	else if(app === "pdf_viewer"){
		this.loadPdfFromURL(file.url, mime_type, filename, strictSSL, function(appInstance) {
			callback(appInstance);
		});
	}
};
...
appLoader.prototype.loadFileFromLocalStorage = function(file, callback) {
	...
	if(app === "image_viewer"){
		this.loadImageFromFile(localPath, mime_type, url, external_url, file.filename, function(appInstance) {
			callback(appInstance);
		});
	}
	else if(app === "movie_player"){
		this.loadVideoFromFile(localPath, mime_type, url, external_url, file.filename, function(appInstance) {
			callback(appInstance);
		});
	}
	else if(app === "pdf_viewer"){
		this.loadPdfFromFile(localPath, mime_type, url, external_url, file.filename, function(appInstance) {
			callback(appInstance);
		});
	}
	else if(app === "custom_app"){
		this.loadAppFromFile(localPath, mime_type, url, external_url, file.filename, function(appInstance) {
			callback(appInstance);
		});
	}
};
...
appLoader.prototype.manageAndLoadUploadedFile = function(file, callback) {
	...
	fs.rename(file.path, localPath, function(err) {
		...
		if(app === "image_viewer"){
			_this.loadImageFromFile(localPath, mime_type, url, external_url, file.originalFilename, function(appInstance) {
				callback(appInstance);
			});
		}
		else if(app === "movie_player"){
			_this.loadVideoFromFile(localPath, mime_type, url, external_url, file.originalFilename, function(appInstance) {
				callback(appInstance);
			});
		}
		else if(app === "pdf_viewer"){
			_this.loadPdfFromFile(localPath, mime_type, url, external_url, file.originalFilename, function(appInstance) {
				callback(appInstance);
			});
		}
		else if(app === "custom_app"){
			...
			_this.loadZipAppFromFile(localPath, mime_type, url, external_url, name, function(appInstance) {
				callback(appInstance);
			});
		}
	});
