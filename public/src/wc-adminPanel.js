...
function initialize() {
	...
	// currently do nothing on connection lost/close
...
function setupListeners() {
	...
	wsio.on('noWebId', function(data) {
		document.getElementById('warnMid').style.visibility = 'visible';
	});
	wsio.on('noWebconPwd', function(data) {
		document.getElementById('warnWebcon').style.visibility = 'visible';
	});
...
function wsGiveClientConfiguration(data) {
	...
	if (data.background.image) {
		...
		if (data.background.image.style) {
			...
			if (data.background.image.style === workingDiv.options[0].value) {
				workingDiv.selectedIndex = 0;
			}
			if (data.background.image.style === workingDiv.options[1].value) {
				workingDiv.selectedIndex = 1;
			}
			if (data.background.image.style === workingDiv.options[2].value) {
				workingDiv.selectedIndex = 2;
			}
		}
	}
...
function removeAlternativeHostEntry() {
	...
	for (var i = 2; i < 15; i++) {
		...
		if (workingDiv == null) {
			...
			// if null on 2 then, there was only 1.
			// the alternative host should by default be 127.0.0.1
...
function sendConfigFileData() {
	...
	// checks if the values do not match,
	// if show show error, then only if there is a value, send it as the new password
	var workingDiv = document.getElementById('meetingIdInput');
	var workingDiv2 = document.getElementById('meetingIdInputConfirm');
	if (workingDiv.value !== workingDiv2.value) {
		document.getElementById('warnMidConfirm').style.visibility = 'visible';
	...
	if (workingDiv.value !== workingDiv2.value) {
		document.getElementById('warnWebconConfirm').style.visibility = 'visible';
		passwordMismatch = true;
	} else if (workingDiv.value.length > 0) {
		data = {};
		data.password = md5(workingDiv.value);
	...
	if (workingDiv.value !== workingDiv2.value) {
		document.getElementById('warnConfigConfirm').style.visibility = 'visible';
		passwordMismatch = true;
	} else if (workingDiv.value.length > 0) {
		data = {};
		data.password = md5(workingDiv.value);
	...
	if (passwordMismatch) {
		...
		document.getElementById('warnPasswordMismatch').style.visibility = 'hidden';
	}
