...
rem echo "Start: CA"
win32\openssl genrsa -des3 -out ca.key  -passout pass:%password% 2048
win32\openssl req -new -key ca.key -sha256 -out ca.csr -passin pass:%password% -subj "/CN=%server%"  -config win32\openssl.cnf
...
win32\openssl genrsa -des3 -out %server%-server.key -passout pass:%password% 2048
win32\openssl req -new -sha256 -key %server%-server.key -out server.csr -passin pass:%password% -subj "/CN=%server%" -config win32\openssl.cnf
rem echo ""
rem echo ""
...
win32\openssl x509 -req -sha256 -days 365 -in server.csr -signkey %server%-server.key -out %server%-server.crt
rem echo ""
rem echo ""
