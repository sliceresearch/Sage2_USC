...
{
    this.drawTWhite(50, 50, "CAVE2 Monitor", "40px");
    ...
    this.drawTWhite(50, 390, "GPU Memory", this.gwin.smallFontSize);
    this.drawTWhite(50, 420, "CPU Cores", this.gwin.smallFontSize);
    this.drawTWhite(50, 550, "RAM", this.gwin.smallFontSize);
    this.drawTWhite(50, 570, "Network Out", this.gwin.smallFontSize);
    this.drawTWhite(50, 590, "Network In", this.gwin.smallFontSize);
},
...
weatherInsideCallback: function(error, datasetTextIn)
{
    ...
    //console.log(parsedCSV[15][0]); // entire line 2 (node 1)
...
drawOneNode: function (x, y, proc, p, row)
{
    ...
    if (p === 2) // processors - value ranges from 0 to 16
        ...
        if (proc < this.glob.temp_colderer)
            this.drawTextRightWhite(x+85, y+12, proc.toFixed(0)+"%", this.gwin.smallFontSize);
        else
...
drawInsideTemp: function ()
{
    ...
    for (j=0; j<37; j+= 1)
    {
        ...
        if (j === 0)    // 0
            {
            this.drawTWhite((j+2)*30-8, 330, "master", this.gwin.smallFontSize);
            this.drawOneNode((j)*30, 300, this.bigD[j][16], 1, -1); // GPU %age
            this.drawOneNode((j)*30, 160, procTotal / 100,  2, -1); // CPU
            this.drawOneNode((j)*30, 140, this.bigD[j][19], 3, -1); // memory %age
            this.drawOneNode((j)*30, 120, this.bigD[j][18], 4, -1); // net out < 1000, 1000 - 100000, > 100000
            this.drawOneNode((j)*30, 100, this.bigD[j][17], 5, -1); // net in < 1000, 1000 - 100000, > 100000
            }

        else if ( (j % 2) === 1) // 1 3 5 7
            {
            this.drawTWhite((j+4)*30+5, 330, j, this.gwin.smallFontSize);
            this.drawOneNode((j+2)*30, 300, this.bigD[j][16], 1, 0); // GPU %age
            this.drawOneNode((j+2)*30, 160, procTotal / 100,  2, 0); // CPU
            this.drawOneNode((j+2)*30, 140, this.bigD[j][19], 3, 0); // memory %age
            this.drawOneNode((j+2)*30, 120, this.bigD[j][18], 4, 0); // net out < 1000, 1000 - 100000, > 100000
            this.drawOneNode((j+2)*30, 100, this.bigD[j][17], 5, 0); // net in < 1000, 1000 - 100000, > 100000
            }

        else if ( (j % 2) === 0) // 2 4 6 8
            {
            this.drawTWhite((j+3)*30+5, 370, j, this.gwin.smallFontSize);
            this.drawOneNode((j+1)*30, 380, this.bigD[j][16], 1, 1); // GPU %age
            this.drawOneNode((j+1)*30, 520, procTotal / 100,  2, 1); // CPU
            this.drawOneNode((j+1)*30, 540, this.bigD[j][19], 3, 1); // memory %age
            this.drawOneNode((j+1)*30, 560, this.bigD[j][18], 4, 1); // net out < 1000, 1000 - 100000, > 100000
            this.drawOneNode((j+1)*30, 580, this.bigD[j][17] ,5, 1); // net in < 1000, 1000 - 100000, > 100000
            }            
        }
},
